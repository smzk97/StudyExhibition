...
function get_main_param(oid,page,pagination_str){
    let ts = Math.round(Date.now() / 1e3).toString();
    let v = ''
    if(page === 1)
        v = `mode=3&oid=${oid}&pagination_str=%7B%22offset%22%3A%22%22%7D&plat=1&seek_rpid=&type=1&web_location=1315875&wts=${ts}`
    else
